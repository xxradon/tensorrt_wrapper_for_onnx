...
def save_weights(graph, file_name):
    ...
    if len(input_tensor) > 1:
        print("only support 1 input!!!\n")
        exit(0)
    dims = input_tensor[0].type.tensor_type.shape.dim
    ...
    weights_info[input_tensor[0].name] = temp
...
if __name__ == "__main__":
    ...
    weights_info = save_weights(graph, "hfnet_github")
    simply_graph = parse_onnx_graph(graph, weights_info)
    #4 generate topology order from simply graph
    topo_order = generate_topo_order(simply_graph["nodes_info"], weights_info)
    #5 generate depend nodes from topo order
    depend_nodes = generate_depend_nodes(topo_order, simply_graph["nodes_info"])

    simply_graph["topo_order"] = topo_order
    simply_graph["depend_nodes"] = depend_nodes
    #5 save weights and simplify graph to files used for tensorrt
